...
def cnn_train():
    ...
    for i in range(0, NUM_CLASSES):
        dir = './train/charsChinese/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
    ...
    val_count = 0
    for i in range(0, NUM_CLASSES):
        dir = './validation/charsChinese/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
    ...
    with tf.Session() as sess:
        ...
        print("一共读取了%s个验证图像，%s个标签" % (val_count, val_count))
        # 设置每次训练op的输入个数和迭代次数，这里为了支持任意图片总数，定义了一个余数remainder,
        # 如果每次训练op的输入个数为60，图片总数为150张，则前面两次各输入60张，最后一次输入30张（余数30）
        batch_size = 60
